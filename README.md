...
- `candidate_recall@50 = 1.0` означает, что на этапе генерации кандидатов нужные статьи попадают в top-50;
- `MAP@10 = 0.1944` показывает, что основная потеря качества происходит на ранжировании top-10;
- поэтому наиболее перспективные направления улучшения - rerank, настройка весов полей и более точная обработка длинных статей.
...
- попробовать chunking длинных статей, чтобы запросы по конкретным фрагментам не терялись внутри большого `body`;
